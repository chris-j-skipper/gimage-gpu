...
void gridVisibilities
			...
			int pStokesProduct,				// the stokes product (used by a-projection only).
...
struct timespec otherStart, otherEnd;
	...
	for ( int wPlane = 0; wPlane < wPlanes; wPlane++ )
		for ( int pbChannel = 0; pbChannel < pbChannels; pbChannel++ )
		{
			...
			for ( int gpu = 0; gpu < pNumGPUs; gpu++ )
				if (phstKernelCache( pbChannel, pStokesProduct, wPlane ).visibilities[ pStageID ][ pBatchID ][ gpu ] > 0)
					foundVis = true;
			if (foundVis == true)
				numKernelSets++;
		}
	...
	for ( int wPlane = 0; wPlane < wPlanes; wPlane++ )
		for ( int pbChannel = 0; pbChannel < pbChannels; pbChannel++ )
		{
			...
			KernelSet & hstKernelSet = phstKernelCache( pbChannel, pStokesProduct, wPlane );
			...
			)
{
	...
	for ( int stokes = 0; stokes < pKernelCache.stokesProducts; stokes++ )
	{
	
		if (pKernelCache.stokesProducts == 1 || (pKernelCache.stokesProducts > 1 && stokes == _param->Stokes))
		{
			printf( "        stokes " );
		...
		else if (pKernelCache.stokesFlag[ stokes ] == true)
		{
			printf( "         stokes " );
			if (stokes == STOKES_I) printf( "I " );
			else if (stokes == STOKES_Q) printf( "Q " );
			else if (stokes == STOKES_U) printf( "U " );
			else if (stokes == STOKES_V) printf( "V " );
		...
		for ( int wPlane = 0; wPlane < pKernelCache.wPlanes; wPlane++ )
			...
			{
				printf( "%i", pKernelCache( pbChannel, /* STOKES = */ 0, wPlane ).supportSize );
...
void hogbomClean( int * pMinorCycle, bool * phstMaskForCleaning, double pHogbomLimit, float ** phstDirtyBeam, float * pdevDirtyImage, VectorI * phstComponentListPos,
			double * phstComponentListValue, int * pComponentListItems, float * phstPrimaryBeam )
{
	...
	reserveGPUMemory( (void **) &devMaxValue, MAX_PIXEL_DATA_AREA_SIZE * sizeof( double ),
						"declaring device memory for psf max pixel value", __LINE__ );
	...
	bool * devMask = NULL;
	if (phstMaskForCleaning != NULL)
	{
		reserveGPUMemory( (void **) &devMask, _param->ImageSize * _param->ImageSize * sizeof( bool ), "creating device memory for the mask", __LINE__ );
		cudaMemcpy( (void *) devMask, phstMaskForCleaning, _param->ImageSize * _param->ImageSize * sizeof( bool ), cudaMemcpyHostToDevice );
	}
...
//tmp = true; // cjs-mod
		...
		moveDeviceToHost( (void *) tmpMaxValue, (void *) devMaxValue, MAX_PIXEL_DATA_AREA_SIZE * sizeof( double ), "copying max pixel data to host", __LINE__ );

		// has the peak value fallen within a specified number of S.D. of the mean? If so, cleaning must stop.
		if (abs( tmpMaxValue[ MAX_PIXEL_VALUE ] ) < pHogbomLimit)
		{
		...
		if (minimumValue >= 0 && abs( tmpMaxValue[ MAX_PIXEL_VALUE ] ) >= (minimumValue * 1.1))
		{
		...
		if (*pMinorCycle >= maxMinorCyclesBeforeFFT && maxMinorCyclesBeforeFFT > -1)
		{
			printf( "\n                %i minor cycles performed in this major cycle. max residual is %6.4e Jy", CYCLE_NITER,
																abs( tmpMaxValue[ MAX_PIXEL_VALUE ] ) );
			break;
		}
		
		// have we reached the required number of iterations without hitting the stopping limit?
		if (abs( tmpMaxValue[ MAX_PIXEL_VALUE ] ) > pHogbomLimit && *pMinorCycle == _param->MinorCycles - 1)
			printf( "\n                clean stopped at %6.4e Jy", abs( tmpMaxValue[ MAX_PIXEL_VALUE ] ) );

		// update the minimum value.
		if (minimumValue < 0.0 || abs( tmpMaxValue[ MAX_PIXEL_VALUE ] ) < minimumValue)
			minimumValue = abs( tmpMaxValue[ MAX_PIXEL_VALUE ] );

		// get the component values.
		int x = (int) round( tmpMaxValue[ MAX_PIXEL_X ] );
		int y = (int) round( tmpMaxValue[ MAX_PIXEL_Y ] );
		double value = tmpMaxValue[ MAX_PIXEL_VALUE ];
		
		// divide the value by the primary-beam ratio.
		if (phstPrimaryBeam != NULL)
		{
			int beamI = (int) round( (double) x * (double) _param->BeamSize / (double) _param->ImageSize );
			int beamJ = (int) round( (double) y * (double) _param->BeamSize / (double) _param->ImageSize );
			float primaryBeam = phstPrimaryBeam[ (beamJ * _param->BeamSize) + beamI ];
			if (primaryBeam < 0.2)
				primaryBeam = 0.2;
//			value /= primaryBeam;
		}
...
//						"copying dirty beam to the device", __LINE__ );
		...
		// subtract dirty beam.
		devAddSubtractBeam<<< _gridSize2D, _blockSize2D >>>(	/* pImage = */ pdevDirtyImage,
									/* pBeam = */ devDirtyBeam,
...
//free( (void *) tmp );
		...
		if (found == -1)
		{
			...
			phstComponentListValue[ *pComponentListItems ] = value * _param->LoopGain;
			(*pComponentListItems)++;
		}
		else
			phstComponentListValue[ found ] += value * _param->LoopGain;
	...
	for ( int stageID = 0; stageID < _hstData[ pMosaicID ].Stages; stageID++ )
	{
		...
		for ( int taylorTerm = 0; taylorTerm < _param->TaylorTerms; taylorTerm++ )
		{
			...
			for ( int gpu = 0; gpu < _param->NumGPUs; gpu++ )
			{
				...
				moveHostToDevice( (void *) devModelImageUV[ gpu ], (void *) phstModelImage[ taylorTerm ], _param->ImageSize * _param->ImageSize * sizeof( cufftComplex ),
							"moving model image to the device", __LINE__ );
			...
			while (hstCurrentVisibility < _hstData[ pMosaicID ].NumVisibilities[ stageID ])
			{
				...
				for ( int wPlane = 0; wPlane < wPlanes; wPlane++ )
					for ( int pbChannel = 0; pbChannel < pbChannels; pbChannel++ )
						for ( int gpu = 0; gpu < _param->NumGPUs; gpu++ )
							visibilitiesInThisBatch += kernelCache( pbChannel, /* STOKES = */ 0, wPlane ).visibilities[ stageID ][ batch ][ gpu ];
				...
				for ( int wPlane = 0; wPlane < wPlanes; wPlane++ )
					for ( int pbChannel = 0; pbChannel < pbChannels; pbChannel++ )
						for ( int cudaDeviceIndex = 0; cudaDeviceIndex < _param->NumGPUs; cudaDeviceIndex++ )
						{
							...
							int visibilitiesInKernelSet = kernelCache( pbChannel, /* STOKES = */ 0, wPlane ).visibilities[ stageID ][ batch ]
																			[ cudaDeviceIndex ];
				...
				// loop over the Stokes products. we need to degrid each product, and create model visibilities.
				for ( int stokesLeakage = 0; stokesLeakage < _param->NumStokesImages; stokesLeakage++ )
				{

					if (_param->NumStokesImages == 1 || (_param->NumStokesImages > 1 && stokesLeakage == _param->Stokes))
					{
					...
					if (_param->AProjection == true && _param->LeakageCorrection == true &&
								_hstData[ pMosaicID ].MuellerMatrixFlag[ (stokesLeakage * _param->NumStokesImages) + _param->Stokes ] == false)
						...
						for ( int wPlane = 0; wPlane < wPlanes; wPlane++ )
							for ( int pbChannel = 0; pbChannel < pbChannels; pbChannel++ )
								for ( int cudaDeviceIndex = 0; cudaDeviceIndex < _param->NumGPUs; cudaDeviceIndex++ )
								{
									...
									int visibilitiesInKernelSet = kernelCache( pbChannel, stokesLeakage, wPlane ).visibilities[ stageID ]
																		[ batch ][ cudaDeviceIndex ];
									if (visibilitiesInKernelSet > 0)
									{
							...
							for ( int pbChannel = 0; pbChannel < pbChannels; pbChannel++ )
								for ( int cudaDeviceIndex = 0; cudaDeviceIndex < _param->NumGPUs; cudaDeviceIndex++ )
								{
									...
									if (visibilitiesInKernelSet > 0)
									{
										...
										moveDeviceToHost( (void *) &hstModelVisibility[ stokesLeakage ][ taylorTerm ]
																	[ visibilityPointer ],
		...
		for ( int taylorTerm = 0; taylorTerm < _param->TaylorTerms; taylorTerm++ )
		{
			...
			while (hstCurrentVisibility < _hstData[ pMosaicID ].NumVisibilities[ stageID ])
			{
				...
				for ( int stokesLeakage = 0; stokesLeakage < _param->NumStokesImages; stokesLeakage++ )
					...
					// ensure the beam pattern for A-projection is not empty.
					if (_param->AProjection == false || _param->LeakageCorrection == false ||
									_hstData[ pMosaicID ].MuellerMatrixFlag[ (stokesLeakage * _param->NumStokesImages) + _param->Stokes ] == true)
					{
								...
								for ( int cudaDeviceIndex = 0; cudaDeviceIndex < _param->NumGPUs; cudaDeviceIndex++ )
								{
									...
									if (visibilitiesInKernelSet > 0)
									{
										...
										moveHostToDevice( (void *) &devModelVisibility_0[ cudaDeviceIndex ]
																	[ hstNextVisibility[ cudaDeviceIndex ] ],
													(void *) &hstModelVisibility[ stokesLeakage ][ /* TAYLOR_TERM = */ 0 ]
																	[ visibilityPointer ],
													visibilitiesInKernelSet * sizeof( cufftComplex ),
										...
										if (_param->Deconvolver == MFS)
										{
											moveHostToDevice( (void *) &devModelVisibility_1[ cudaDeviceIndex ]
																[ hstNextVisibility[ cudaDeviceIndex ] ],
														(void *) &hstModelVisibility[ stokesLeakage ]
											...
											moveHostToDevice( (void *) &devMfsWeight_1[ cudaDeviceIndex ]
																[ hstNextVisibility[ cudaDeviceIndex ] ],
													(void *) &_hstData[ pMosaicID ].MfsWeight[ stokesLeakage ]
						...
						for ( int wPlane = 0; wPlane < wPlanes; wPlane++ )
							for ( int pbChannel = 0; pbChannel < pbChannels; pbChannel++ )
								for ( int cudaDeviceIndex = 0; cudaDeviceIndex < _param->NumGPUs; cudaDeviceIndex++ )
								{
									...
									int visibilitiesInKernelSet = kernelCache( pbChannel, /* STOKES = */ 0, wPlane ).visibilities[ stageID ]
																		[ batch ][ cudaDeviceIndex ];
									if (visibilitiesInKernelSet > 0)
									{
										...
										moveDeviceToHost( (void *) &_hstData[ pMosaicID ].ResidualVisibility[ stokesLeakage ]
														[ taylorTerm ][ visibilityPointer ],
	...
	for ( int stageID = 0; stageID < _hstData[ pMosaicID ].Stages; stageID++ )
	{
		...
		while (hstCurrentVisibility < _hstData[ pMosaicID ].NumVisibilities[ stageID ])
		{
			...
			for ( int stokesLeakage = 0; stokesLeakage < _param->NumStokesImages; stokesLeakage++ )
			{
				...
				// ensure the beam pattern for A-projection is not empty.
				if (_param->AProjection == true && _hstData[ pMosaicID ].InverseMuellerMatrixFlag[ (_param->Stokes * 4) + stokesLeakage ] == false)
...
void generateDirtyImages
			...
			char ** pFilename,				// an array of dirty image filenames, one for each Taylor term
			bool * phstMask,				//
			bool pSaveDirtyImages,				// true if we need to save the dirty images
			...
			)
{
	...
	if (_param->Stokes == STOKES_I) printf( "Stokes I " );
	else if (_param->Stokes == STOKES_Q) printf( "Stokes Q " );
	else if (_param->Stokes == STOKES_U) printf( "Stokes U " );
	else if (_param->Stokes == STOKES_V) printf( "Stokes V " );
	printf( "dirty image" );
//	if (_param->Deconvolver == MFS)
//		printf( " (Taylor term %i)", taylorTerm );
	printf( ".....\n\n" );
			...
			if (_param->UvPlaneMosaic == false || image == _hstData.size() - 1)
			{
				...
				if (_param->NumGPUs > 1)
				{
					...
					for ( int gpu = 1; gpu < _param->NumGPUs; gpu++ )
					{
						...
						cudaSetDevice( _param->GPU[ gpu ] );
						moveDeviceToHost( (void *) hstTmpImage, devDirtyImageGrid[ gpu ], _param->ImageSize * _param->ImageSize * sizeof( cufftComplex ),
...
//}
				...
				// cjs-mod if (_param->UseMosaicing == false && _param->AProjection == false)
				{
					...
					moveHostToDevice( (void *) devPrimaryBeamRatio, (void *) _hstPrimaryBeamRatioPattern, // cjs-mod _hstData[ 0 ].PrimaryBeamRatio
	...
	if (pSaveDirtyImages == true)
		for ( int taylorTerm = 0; taylorTerm < _param->TaylorTerms; taylorTerm++ )
			_hstCasacoreInterface.WriteCasaImage(	/* pFilename = */ pFilename[ taylorTerm ],
								...
								/* pImage = */ phstDirtyImage[ taylorTerm ],
								/* pFrequency = */ CONST_C / _hstData[ /* MOSAIC_COMPONENT = */ 0 ].AverageWavelength,
								/* pMask = */ phstMask,
								/* pDirectionType = */ CasacoreInterface::J2000,
								/* pStokesImages = */ 1 );
...
bool cottonSchwabClean
			...
			bool * phstMaskForCleaning,			// the 10% beam-level mask
			...
			)
{
	...
	if (_param->Deconvolver == MFS)
	{
		...
		for ( long int i = 0; i < (long int) _param->ImageSize * (long int) _param->ImageSize; i++ )
		{
			...
			if (phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ] > maxResidual && pixelOK == true)
				maxResidual = abs( phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ] );
		}

		// create the MFS mask.
		hstMfsMask = (bool *) malloc( (long int) _param->ImageSize * (long int) _param->ImageSize * (long int) sizeof( bool ) );
		for ( long int i = 0; i < (long int) _param->ImageSize * (long int) _param->ImageSize; i++ )
			hstMfsMask[ i ] = phstMask[ i ] && (phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ] >= maxResidual / 10.0);
	...
	for ( int i = 0; i < _param->TaylorTerms; i++ )
		hstComponentListValue[ i ] = (double *) malloc( _param->MinorCycles * sizeof( double ) );
...
//break;
		...
		// multiply the dirty image by the primary-beam pattern.
		for ( int i = 0; i < _param->ImageSize; i++ )
			for ( int j = 0; j < _param->ImageSize; j++ )
			{
		
				int beamI = (int) round( (double) i * (double) _param->BeamSize / (double) _param->ImageSize );
				int beamJ = (int) round( (double) j * (double) _param->BeamSize / (double) _param->ImageSize );
	
//				phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ (j * _param->ImageSize) + i ] *= _hstPrimaryBeamPattern[ (beamJ * _param->BeamSize) + beamI ];
		...
		for ( long int i = 0; i < (long int) _param->ImageSize * (long int) _param->ImageSize; i++ )
		{
			bool pixelOK = true;
			if (phstMaskForCleaning != NULL)
				pixelOK = (phstMaskForCleaning[ i ] == true);
			if (abs( phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ] ) > maxResidual && pixelOK == true)
				maxResidual = abs( phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ] );
		}
...
//		}
		...
		else
		{
			...
			if (_param->Deconvolver == HOGBOM)
			{
				...
				moveHostToDevice( (void *) devDirtyImage, (void *) phstDirtyImage[ /* TAYLOR_TERM = */ 0 ],
				...
				hogbomClean(	/* pMinorCycle = */ &numMinorCycles,
						/* phstMaskForCleaning = */ phstMaskForCleaning,
						...
						/* phstComponentListValue = */ hstComponentListValue[ /* TAYLOR_TERM = */ 0 ],
			...
			if (_param->Deconvolver == MFS)
			{
				...
				for ( int t = 0; t < _param->TaylorTerms; t++ )
				{
					...
					reserveGPUMemory( (void **) &devDirtyImage[ t ], _param->ImageSize * _param->ImageSize * sizeof( float ),
								"reserving device memory for dirty image (cleaning)", __LINE__ );
					moveHostToDevice( (void *) devDirtyImage[ t ], (void *) phstDirtyImage[ t ],
								_param->ImageSize * _param->ImageSize * sizeof( float ), "copying dirty image to the device", __LINE__ );
				}
...
//						/* pComponentListItems = */ &numComponents );
			...
			// some more.
			if (numMinorCycles < _param->MinorCycles && hogbomLimit != _param->Threshold)
				reachedLimit = false;
		...
		for ( int t = 0; t < _param->TaylorTerms; t++ )
		{
			if (phstDirtyImage[ t ] != NULL)
				free( (void *) phstDirtyImage[ t ] );
			phstDirtyImage[ t ] = NULL;
		}
		...
		quickSortComponents(	/* phstComponentListPos = */ hstComponentListPos,
					...
					/* pTaylorTerms = */ _param->TaylorTerms );

		// -------------------------------------------------------------------
		//
		// S T E P   2 :   C O N T R U C T   M O D E L   I M A G E   F O R
		//		   E A C H   T A Y L O R   T E R M
		//
		// -------------------------------------------------------------------
		...
		for ( int taylorTerm = 0; taylorTerm < _param->TaylorTerms; taylorTerm++ )
		{
			...
			reserveGPUMemory( (void **) &devComponentValue, numComponents * sizeof( double ), "reserving device memory for clean components", __LINE__ );
			moveHostToDevice( (void *) devComponentValue, (void *) hstComponentListValue[ taylorTerm ], numComponents * sizeof( double ),
							"moving component list values to the device", __LINE__ );

			// upload the component positions to the device.
			VectorI * devComponentPos = NULL;
			reserveGPUMemory( (void **) &devComponentPos, numComponents * sizeof( VectorI ), "reserving device memory for clean component positions", __LINE__ );
			moveHostToDevice( (void *) devComponentPos, (void *) hstComponentListPos, numComponents * sizeof( VectorI ),
...
//char filename[100];
//sprintf( filename, "model-image" );
//float * hst = (float *) malloc( _param->ImageSize * _param->ImageSize * sizeof( float ) );
//moveDeviceToHost( (void *) hst, (void *) devModelImage, _param->ImageSize * _param->ImageSize * sizeof( float ), "---", __LINE__ );
//_hstCasacoreInterface.WriteCasaImage( filename, _param->ImageSize, _param->ImageSize, _param->OutputRA, _param->OutputDEC, _param->CellSize, hst, CONST_C / _hstAverageWavelength[ 0 ], NULL );
//free( (void *) hst );
//abort();
...
//}
			...
			hstMosaicModel[ taylorTerm ] = NULL;
			hstModelImageUV[ taylorTerm ] = (cufftComplex *) malloc( _param->ImageSize * _param->ImageSize * sizeof( cufftComplex ) );
			if (_param->ImagePlaneMosaic == true)
			{
				hstMosaicModel[ taylorTerm ] = (float *) malloc( _param->ImageSize * _param->ImageSize * sizeof( float ) );
				moveDeviceToHost( (void *) hstMosaicModel[ taylorTerm ], (void *) devModelImage, _param->ImageSize * _param->ImageSize * sizeof( float ),
											"copying mosaic model to the host", __LINE__ );
			}	
			else
			{
				...
				moveDeviceToHost( (void *) hstModelImageUV[ taylorTerm ], (void *) devModelImage, _param->ImageSize * _param->ImageSize * sizeof( cufftComplex ),
											"copying model image to the host", __LINE__ );
...
//char filename[100];
//sprintf( filename, "model-image-fft" );
//float * hst = (float *) malloc( _param->ImageSize * _param->ImageSize * sizeof( float ) );
//moveDeviceToHost( (void *) hst, (void *) devModelImage, _param->ImageSize * _param->ImageSize * sizeof( float ), "---", __LINE__ );
//_hstCasacoreInterface.WriteCasaImage( filename, _param->ImageSize, _param->ImageSize, _param->OutputRA, _param->OutputDEC, _param->CellSize, hst, CONST_C / _hstAverageWavelength[ 0 ], NULL );
//free( (void *) hst );
...
//}
		...
		for ( int image = 0; image < _hstData.size(); image++ )
		{
			...
			if (_param->ImagePlaneMosaic == true)
			{
				...
				for ( int taylorTerm = 0; taylorTerm < _param->TaylorTerms; taylorTerm++ )
				{
					...
					extractFromMosaic(	/* pdevImage = */ devModelImage,
								/* phstMosaic = */ hstMosaicModel[ taylorTerm ],
								/* phstMask = */ phstMask,
								/* pImageID = */ image );
			...
			// degrid the visibilities from this image, and subtract the model visibilities from the original visibilities.
		...
		{
			for ( int t = 0; t < _param->TaylorTerms; t++ )
				if (hstModelImageUV[ t ] != NULL)
					free( (void *) hstModelImageUV[ t ] );
		...
		generateDirtyImages(	/* phstDirtyImage = */ phstDirtyImage,
					/* pFilename = */ NULL,
					/* phstMask = */ phstMask,
					/* pSaveDirtyImages = */ false,
	...
	for ( int t = 0; t < _param->TaylorTerms; t++ )
		...
		_hstCasacoreInterface.WriteCasaImage(	/* pFilename = */ pResidualImageFilename[ t ],
							/* pWidth = */ _param->ImageSize,
							/* pHeight = */ _param->ImageSize,
							/* pRA = */ _param->OutputRA,
							/* pDec = */ _param->OutputDEC,
							/* pPixelSize = */ _param->CellSize,
							/* pImage = */ phstDirtyImage[ t ],
							/* pFrequency = */ CONST_C / _hstData[ /* MOSAIC_COMPONENT = */ 0 ].AverageWavelength,
							/* pMask = */ phstMask,
							/* pDirectionType = */ CasacoreInterface::J2000,
							/* pStokesImages = */ 1 );
	...
	for ( int taylorTerm = 0; taylorTerm < _param->TaylorTerms; taylorTerm++ )
	{
		reserveGPUMemory( (void **) &devComponentValue[ taylorTerm ], numComponents * sizeof( double ), "reserving device memory for clean components", __LINE__ );
		moveHostToDevice( (void *) devComponentValue[ taylorTerm ], (void *) hstComponentListValue[ taylorTerm ], numComponents * sizeof( double ),
				"moving component list values to the device", __LINE__ );
	}
	...
	VectorI * devComponentPos = NULL;
	reserveGPUMemory( (void **) &devComponentPos, numComponents * sizeof( VectorI ), "reserving device memory for clean component positions", __LINE__ );
	moveHostToDevice( (void *) devComponentPos, (void *) hstComponentListPos, numComponents * sizeof( VectorI ), "moving component list positions to the device", __LINE__ );
	...
	{
		for ( int i = 0; i < _param->TaylorTerms; i++ )
			if (hstComponentListValue[ i ] != NULL)
				free( (void *) hstComponentListValue[ i ] );
	...
	for ( int taylorTerm = 0; taylorTerm < _param->TaylorTerms; taylorTerm++ )
	{
		...
		moveHostToDevice( (void *) devCleanImage, (void *) phstDirtyImage[ taylorTerm ], _param->ImageSize * _param->ImageSize * sizeof( float ),
		...
		// if we're using a-projection then apply the primary-beam pattern to the clean image.
//		if (_hstPrimaryBeamPattern != NULL && _param->AProjection == true)
//		{

//			float * devPrimaryBeamPattern = NULL;
//			reserveGPUMemory( (void **) &devPrimaryBeamPattern, _param->BeamSize * _param->BeamSize * sizeof( float ),
//						"declaring device memory for primary beam pattern", __LINE__ );
//			moveHostToDevice( (void *) devPrimaryBeamPattern, (void *) _hstPrimaryBeamPattern,
//						_param->BeamSize * _param->BeamSize * sizeof( float ), "copying primary beam pattern to the device", __LINE__ );

			// apply the primary beam pattern
//			devMultiplyImages<<< _gridSize2D, _blockSize2D >>>(	/* pOne = */ devCleanImage,
//										/* pTwo = */ devPrimaryBeamPattern,
//										/* pMask = */ NULL,
//										/* pSizeOne = */ _param->ImageSize,
//										/* pSizeTwo = */ _param->BeamSize );
...
//		}
		...
		moveDeviceToHost( (void *) phstDirtyImage[ taylorTerm ], (void *) devCleanImage, _param->ImageSize * _param->ImageSize * sizeof( float ),
	...
	if (_param->Deconvolver == MFS && _param->TaylorTerms > 1)
	{
		...
		// divide tt1 by tt0.
		for ( int i = 0; i < _param->ImageSize * _param->ImageSize; i++ )
			if (phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ] != 0.0)
				phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ] = phstDirtyImage[ /* TAYLOR_TERM = */ 1 ][ i ] / phstDirtyImage[ /* TAYLOR_TERM = */ 0 ][ i ];
		...
		_hstCasacoreInterface.WriteCasaImage(	/* pFilename = */ pAlphaImageFilename,
							...
							/* pDec = */ _param->OutputDEC,
							/* pPixelSize = */ _param->CellSize,
							/* pImage = */ phstDirtyImage[ /* TAYLOR_TERM = */ 0 ],
							/* pFrequency = */ CONST_C / _hstData[ /* MOSAIC_COMPONENT = */ 0 ].AverageWavelength,
							/* pMask = */ hstMfsMask,
							/* pDirectionType = */ CasacoreInterface::J2000,
							/* pStokesImages = */ 1 );
		...
		for ( int mosaicID = 0; mosaicID < _hstData.size(); mosaicID++ )
		{
			...
			for ( int stageID = 0; stageID < _hstData[ mosaicID ].Stages; stageID++ )
			{
				...
				while (hstCurrentVisibility < _hstData[ mosaicID ].NumVisibilities[ stageID ])
				{
					...
					for ( int wPlane = 0; wPlane < wPlanes; wPlane++ )
							for ( int gpu = 0; gpu < _param->NumGPUs; gpu++ )
								visibilitiesInThisBatch += kernelCache( /* CHANNEL = */ 0, /* STOKES = */ 0, wPlane ).visibilities[ stageID ]
																				[ batch ][ gpu ];
					...
					for ( int wPlane = 0; wPlane < _param->WPlanes; wPlane++ )
					{
						...
						int visibilitiesInKernelSet = kernelCache( /* CHANNEL = */ 0, /* STOKES = */ 0, wPlane ).visibilities[ stageID ][ batch ]
																			[ cudaDeviceIndex ];
			...
			// divide the dirty beam by the deconvolution image to remove the anti-aliasing kernel.
...
double ** getPolarisationMultiplier( char * pMeasurementSetFilename, int * pNumPolarisations, int * pNumPolarisationConfigurations, char * pTableData )
{
	...
	{
		hstMultiplier = (double **) malloc( _param->NumStokesImages * sizeof( double * ) );
		for ( int s = 0; s < _param->NumStokesImages; s++ )
		{
	...
	if (*pNumPolarisations >= 1)
		for ( int config = 0; config < (*pNumPolarisationConfigurations); config++ )
		{
			...
			for ( int stokesProduct = 0; stokesProduct < _param->NumStokesImages; stokesProduct++ )
			{
				...
				int whichStokes = _param->Stokes;
				bool requestedImage = true;
				if (_param->AProjection == true && _param->LeakageCorrection == true)
				{
					whichStokes = stokesProduct;
					requestedImage = (whichStokes == _param->Stokes);
				}
...
void buildMaskAndPrimaryBeamPattern( bool * phstMask, bool * phstMaskForCleaning, char * pPrimaryBeamPatternFilename )
{

	const double PB_MASK_THRESHOLD = 0.2;
	const double CLEANING_THRESHOLD = 0.10;
	...
	memset( phstMaskForCleaning, 0, (long int) _param->ImageSize * (long int) _param->ImageSize * sizeof( bool ) );
	...
	if (_param->UseMosaicing == false)
	{
		...
		for ( long j = 0; j < _param->ImageSize; j++ )
		{
			...
			for ( int i = 0; i < _param->ImageSize; i++, index++ )
			{
				...
				double beam = interpolateBeam(	/* pBeam = */ _hstData[ /* MOSAIC_ID = */ 0 ].PrimaryBeam,
									/* pBeamSize = */ _param->BeamSize,
									/* pI = */ beamI,
									/* pJ = */ beamJ,
									/* pFracI = */ fracI,
				...
				phstMaskForCleaning[ index ] = (beam >= CLEANING_THRESHOLD);
			}
		}
		...
		if (_param->UseMosaicing == true)
		{
			...
			for ( int j = 0; j < _param->ImageSize; j++ )
			{
				...
				for ( int i = 0; i < _param->ImageSize; i++, index++ )
				{
					int beamI = (int) floor( (double) i / imageScale );
					double fracI = ((double) i / imageScale) - (double) beamI;
					double beam = interpolateBeam(	/* pBeam = */ _hstPrimaryBeamPattern,
										/* pBeamSize = */ _param->BeamSize,
										/* pI = */ beamI,
										/* pJ = */ beamJ,
										/* pFracI = */ fracI,
...
void generateKernelCache()
{
	...
	for ( int mosaicID = 0; mosaicID < _hstData.size(); mosaicID++ )
	{
		...
		if (_param->AProjection == false)
		{
			printf( "reprojecting primary beam at wavelength %6.4f mm\n", _hstData[ mosaicID ].MaximumWavelength * 1000.0 );
		...
		if (_param->UvPlaneMosaic == true || _param->AProjection == true)
		{
		...
		printf( "mosaic component %i, gridding ", mosaicID );
		if (_param->MinorCycles > 0)
			printf( "and degridding " );
		printf( "cache(s):\n" );
		_griddingKernelCache.push_back( newKernelCache );
		_griddingKernelCache[ mosaicID ].Create(	/* pPBChannels = */ 1, // cjs-mod (_param->AProjection == true ? _param->PBChannels : 1),
								/* pWPlanes = */ _param->WPlanes,
								/* pWProjection = */ _param->WProjection,
		...
		if (_param->MinorCycles > 0)
		{
			...
			_degriddingKernelCache[ mosaicID ].Create(	/* pPBChannels = */ _param->PBChannels,
									/* pWPlanes = */ _param->WPlanes,
									/* pWProjection = */ _param->WProjection,
									/* pAProjection = */ _param->AProjection,
									/* pUseMosaicing = */ _param->UseMosaicing,
									...
									/* phstPrimaryBeamMosaicing = */ hstPrimaryBeamAverageWavelength );
		}
		...
		for ( int pbChannel = 0; pbChannel < _param->PBChannels; pbChannel++ )
		{
			...
			if (_param->AProjection == true)
			{
				...
				_hstData[ mosaicID ].GenerateMuellerMatrix(	/* pPBChannel = */ pbChannel,
										...
										/* pdevPrimaryBeam = */ devPrimaryBeamAverageWavelength,
										/* pPBMaxValue = */ pbMaxValue );
...
for ( int i = 0; i < OVERSAMPLED_BEAM_SIZE * OVERSAMPLED_BEAM_SIZE; i++ )
	mueller[ i ] = _hstData[ mosaicID ].MuellerMatrix[ 0 ][ i ].x;
char filename[100];
sprintf( filename, "mueller-0" );
...
if (mueller != NULL)
	free( (void *) mueller );
				...
				// generate gridding kernels.
// cjs-mod				_griddingKernelCache[ mosaicID ].GenerateKernelCache( /* pPBChannel = */ pbChannel );
				
			}
				
			// generate degridding kernels (if cleaning is required).
			if (_param->MinorCycles > 0)
...
//{
//char filename[100];
//sprintf( filename, "primary-beam-ratio-200" );
//_hstCasacoreInterface.WriteCasaImage( filename, OVERSAMPLED_BEAM_SIZE, OVERSAMPLED_BEAM_SIZE, _param->OutputRA, _param->OutputDEC, _param->CellSize, 
//					_hstData[ mosaicID ].PrimaryBeamRatio, CONST_C / _hstData[ 0 ].AverageWavelength, NULL, CasacoreInterface::J2000, 1 );
//}
...
// cjs-mod 		if (_param->AProjection == false)
		...
		if (_param->MinorCycles > 0)
		{
			...
			_psfKernelCache[ mosaicID ].Create(	/* pPBChannels = */ 1,
								/* pWPlanes = */ _param->WPlanes,
								/* pWProjection = */ _param->WProjection,
								/* pAProjection = */ false,
								/* pUseMosaicing = */ false,
	...
	printf( "prolate-spheroidal function:\n" );
...
int main( int pArgc, char ** pArgv )
{
	...
	// check that leakage correction is turned off for generated primary beams (we'll only have generated the diagonal elements of the Mueller).
	if (_param->BeamPattern[ /* MOSAIC_COMPONENT = */ 0 ][0] == '\0')
		_param->LeakageCorrection = false;
					
	// is thie primary beam large enough? if not we need to scale it up.
	...
	_param->NumStokesImages = (_param->AProjection == true && _param->LeakageCorrection == true ? 4 : 1);
...
//	_param->GPU[ 0 ] = atoi( _param->GPUParam );
	...
	for ( int measurementSet = 0; measurementSet < _param->MeasurementSets; measurementSet++ )
//		processMeasurementSet(	/* pFilenamePrefix = */ _param->OutputPrefix,
//					/* pMeasurementSetFilename = */ _param->MeasurementSetPath[ measurementSet ],
...
//					/* phstTotalWeightPerCell = */ hstTotalWeightPerCell );
	...
	bool * hstMaskForCleaning = (bool *) malloc( (long int) _param->ImageSize * (long int) _param->ImageSize * sizeof( bool ) );
	buildMaskAndPrimaryBeamPattern(	/* phstMask = */ hstMask,
						/* phstMaskForCleaning = */ hstMaskForCleaning,
...
//}
	...
	for ( int t = 0; t < _param->TaylorTerms; t++ )
		hstDirtyImage[ /* TAYLOR_TERM = */ t ] = NULL;

	// generate dirty images for this Stokes parameter (one for each Taylor term).
	generateDirtyImages(	/* phstDirtyImage = */ hstDirtyImage,
				/* pFilename = */ outputDirtyImageFilename,
				/* phstMask = */ hstMask,
				/* pSaveDirtyImages = */ true,
	...
	if (_param->MinorCycles > 0)
	{
		...
		cottonSchwabClean(	/* pdevCleanBeam = */ devCleanBeam,
					...
					/* phstMask = */ hstMask,
					/* phstMaskForCleaning = */ NULL, // cjs-mod hstMaskForCleaning,
	...
	{
		for ( int t = 0; t < _param->TaylorTerms; t++ )
			if (hstDirtyImage[ t ] != NULL)
				free( (void *) hstDirtyImage[ t ] );
		free( (void *) hstDirtyImage );
	}
	if (hstMask != NULL)
		free( (void *) hstMask );
	if (hstMaskForCleaning != NULL)
		free( (void *) hstMaskForCleaning );
