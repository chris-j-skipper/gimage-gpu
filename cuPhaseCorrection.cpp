...
//	is shifted around the celestial sphere to a new position. A Euler rotation is also applied to the uvw coordinates. This method
//	is initiated by setting the 'uv_projection' parameter in the 'uvfacetting-params' file to 'N'.
//
//	The second method (which we call image plane facetting) also uses a phase correction and Euler rotation, but in addition will
//	apply a further 2D transformation to the uvw coordinates in order that the resulting image is projected onto a new tangential
//	plane centred on the new phase position. This process is equivalent to an image-plane reprojection done in uv space. This
