...
char _hstBeamPattern[1024] = "\0";			// the file or files with the primary beam patterns.
...
// texture maps.
cudaChannelFormatDesc _channelDesc = cudaCreateChannelDesc( 32, 0, 0, 0, cudaChannelFormatKindFloat );
...
__global__ void devGridVisibilities( cufftComplex * pGrid, cufftComplex * pVisibility, int pVisibilitiesPerBlock, int pBlocksPerVisibility,
					VectorI * pGridPosition, int * pKernelIndex, float * pWeight, int pNumVisibilities,
					int pNumKernels, int pSize, bool pComplex )
{
...
__global__ void devDegridVisibilities( cufftComplex * pGrid, cufftComplex * pVisibility, VectorI * pGridPosition, int * pKernelIndex,
					int pNumVisibilities, int pSize, int pVisibilitiesPerPage )
{
	...
	if ( kernelX < _devKernelSize && kernelY < _devKernelSize && visibilityIndex < pNumVisibilities )
	{
		...
		int kernelIndex = pKernelIndex[ visibilityIndex ];
					
		// get kernel value.
		cufftComplex kernel;
		kernel.x = tex3D( _kernelTextureReal, kernelX, kernelY, kernelIndex );
		kernel.y = tex3D( _kernelTextureImag, kernelX, kernelY, kernelIndex );
...
void createCUDAArrayForKernels( cufftComplex * pdevKernel, int pKernelSize, int pNumKernels )
{
	...
	// construct a float pointer to the kernel.
	float * devKernelPtr = (float *) pdevKernel;
	
	// set up texture mapping. we can only create double-type texture maps, so
	// we put the real and imaginary components in separate texture maps.
	// create the structures for copying data to 3D arrays.
	cudaMemcpy3DParms copyParams = {0};
	copyParams.srcPtr = make_cudaPitchedPtr( (void *) devKernelPtr, pKernelSize * sizeof( float ), pKernelSize, pKernelSize );
	...
	copyParams.srcPtr = make_cudaPitchedPtr( (void *) &devKernelPtr[ pKernelSize * pKernelSize * pNumKernels ],
							pKernelSize * sizeof( float ), pKernelSize, pKernelSize );
...
void createCUDAArrayForKernels( float * pdevKernel, int pKernelSize, int pNumKernels )
{
	...
	// create the structures for copying data to 3D arrays.
	cudaMemcpy3DParms copyParams = {0};
	copyParams.srcPtr = make_cudaPitchedPtr( (void *) pdevKernel, pKernelSize * sizeof( float ), pKernelSize, pKernelSize );
	copyParams.dstArray = _devKernelArrayRealPtr;
	copyParams.extent = arraySize;
	copyParams.kind = cudaMemcpyDeviceToDevice;
...
void rearrangeKernel( cufftComplex * pKernel, long int pSize )
{
	...
	ok = reserveGPUMemory( (void **) &devTmp, pSize * sizeof( cufftComplex ), "creating temporary kernel memory on the device" );
	if (ok == true)
	{

		// copy kernel to temporary memory.
		err = cudaMemcpy( devTmp, pKernel, pSize * sizeof( cufftComplex ), cudaMemcpyDeviceToDevice );
		if (err != cudaSuccess)
		{
	...
	if (ok == true)
	{
		...
		devRearrangeKernel<<< blocks, threads >>>(	/* pTarget = */ (float *) pKernel,
								/* pSource = */ (float *) devTmp,
								/* pElements = */ pSize );
...
void getParameters( char * pParameterFile )
{
	...
	_hstMosaicID = (int *) malloc( sizeof( int ) );
	_hstMosaicID[ 0 ] = -1;
	...
	while (fgets( line, 1024, fr ) != NULL)
	{
		...
		if (strcmp( par, MEASUREMENT_SET ) == 0)
		{
			...
			_hstMosaicID = (int *) realloc( _hstMosaicID, _hstMeasurementSets * sizeof( int ) );
			_hstMosaicID[ _hstMeasurementSets - 1 ] = -1;
...
//			strcpy( _hstDataField, params );
		...
		else if (strcmp( par, FILE_ID ) == 0 && _hstMeasurementSets > 0)
		{

			// ensure we've read a path name already.
			if (_hstMeasurementSetPath[ _hstMeasurementSets - 1 ] != NULL)
			{
...
//				_hstMeasurementSets = 0;
				...
				// store the measurement set filename, the field id, the spw and the data field.
				char tmpPath[1024] = "\0", tmpFieldID[1024] = "\0", tmpSpw[1024] = "\0", tmpDataField[1024] = "\0", tmpTableData[1024] = "\0";
				strcpy( tmpPath, _hstMeasurementSetPath[ _hstMeasurementSets - 1 ] );
				strcpy( tmpFieldID, _hstFieldID[ _hstMeasurementSets - 1 ] );
				strcpy( tmpSpw, _hstSpwRestriction[ _hstMeasurementSets - 1 ] );
				strcpy( tmpDataField, _hstDataField[ _hstMeasurementSets - 1 ] );
				strcpy( tmpTableData, _hstTableData[ _hstMeasurementSets - 1 ] );

				// decrement the number of measurement sets because we'll replace the wildcard name.
				free( (void *) _hstMeasurementSetPath[ _hstMeasurementSets - 1 ] );
				free( (void *) _hstFieldID[ _hstMeasurementSets - 1 ] );
				free( (void *) _hstSpwRestriction[ _hstMeasurementSets - 1 ] );
				free( (void *) _hstDataField[ _hstMeasurementSets - 1 ] );
				free( (void *) _hstTableData[ _hstMeasurementSets - 1 ] );
				_hstMeasurementSets--;
				...
				char tmp[ 512 ];
				strcpy( tmp, params );
				int newMeasurementSets = 0;
				while ((fileID = strtok( (newMeasurementSets > 0 ? NULL : tmp), " ," )) != NULL)
					newMeasurementSets++;

				minMeasurementSet = _hstMeasurementSets;

				// if no file ids are found then we'll just add the wildcard one back in.
				if (newMeasurementSets == 0)
				{
					_hstMeasurementSets++;
					_hstMeasurementSetPath = (char **) realloc( _hstMeasurementSetPath, _hstMeasurementSets * sizeof( char * ) );
					_hstMeasurementSetPath[ _hstMeasurementSets ] = (char *) malloc( 1024 * sizeof( char ) );
					strcpy( _hstMeasurementSetPath[ _hstMeasurementSets - 1 ], tmpPath );
					_hstMosaicID = (int *) realloc( _hstMosaicID, _hstMeasurementSets * sizeof( int ) );
					_hstMosaicID[ _hstMeasurementSets - 1 ] = -1;
					maxMeasurementSet = minMeasurementSet;
				}
				else
				{

					// reserve memory for these file IDs.
					_hstMosaicID = (int *) realloc( _hstMosaicID, (_hstMeasurementSets + newMeasurementSets) * sizeof( int ) );
			
					// read file IDs.
					int i = 0;
					while ((fileID = strtok( (i > 0 ? NULL : params), " ," )) != NULL)
					{
						_hstMosaicID[ _hstMeasurementSets + i ] = atoi( fileID );
						i++;
					}

					// populate filenames.
					_hstMeasurementSetPath = (char **) realloc( _hstMeasurementSetPath, (_hstMeasurementSets + newMeasurementSets) * sizeof( char * ) );
					for ( int i = 0; i < newMeasurementSets; i++ )
					{
						_hstMeasurementSetPath[ _hstMeasurementSets + i ] = (char *) malloc( 1024 * sizeof( char ) );
						strcpy( _hstMeasurementSetPath[ _hstMeasurementSets + i ], tmpPath );
					}
	
					// do the replacement of the wildcards within the filenames.
					for ( int i = 0; i < newMeasurementSets; i++ )
					{

						// build the filename by replacing the '**' characters in the measurement set by the index.
						bool replacementMade = false;
						char * filename = _hstMeasurementSetPath[ _hstMeasurementSets + i ];
						for ( int j = 0; j < strlen( filename ) - 1; j++ )
							if (filename[ j ] == '*' && filename[ j + 1 ] == '*')
							{
								filename[ j ] = ((_hstMosaicID[ _hstMeasurementSets + i ] / 10) % 10) + '0';
								filename[ j + 1 ] = (_hstMosaicID[ _hstMeasurementSets + i ] % 10) + '0';
						...
						if (replacementMade == false)
							for ( int j = 0; j < strlen( filename ); j++ )
								if (filename[ j ] == '*')
								{
									filename[ j ] = (_hstMosaicID[ _hstMeasurementSets + i ] % 10) + '0';
									replacementMade = true;
									break;
								}
					...
					_hstMeasurementSets += newMeasurementSets;
					maxMeasurementSet = minMeasurementSet + newMeasurementSets - 1;
				...
				for ( int i = minMeasurementSet; i <= maxMeasurementSet; i++ )
				{
					_hstFieldID[ i ] = (char *) malloc( 1024 * sizeof( char ) );
					strcpy( _hstFieldID[ i ], tmpFieldID );
				}

				// update the spw restrictions.
				_hstSpwRestriction = (char **) realloc( _hstSpwRestriction, _hstMeasurementSets * sizeof( char * ) );
				for ( int i = minMeasurementSet; i <= maxMeasurementSet; i++ )
				{
					_hstSpwRestriction[ i ] = (char *) malloc( 1024 * sizeof( char ) );
					strcpy( _hstSpwRestriction[ i ], tmpSpw );
				}
				...
				for ( int i = minMeasurementSet; i <= maxMeasurementSet; i++ )
				{
					_hstDataField[ i ] = (char *) malloc( 1024 * sizeof( char ) );
					strcpy( _hstDataField[ i ], tmpDataField );
				}

				// update the table data path.
				_hstTableData = (char **) realloc( _hstTableData, _hstMeasurementSets * sizeof( char * ) );
				for ( int i = minMeasurementSet; i <= maxMeasurementSet; i++ )
				{
					_hstTableData[ i ] = (char *) malloc( 1024 * sizeof( char ) );
					strcpy( _hstTableData[ i ], tmpTableData );
				}
...
//	cudaError_t errMem = cudaMemGetInfo( &freeMem, &totalMem );
//	if (errMem == cudaSuccess)
//		printf( "cudaMemGetInfo succeeded at %d\n", __LINE__ );
//	else
//		printf( "cudaMemGetInfo failed at %d (%s)\n", __LINE__, cudaGetErrorString( errMem ) );
//}
				...
				reserveGPUMemory( (void **) &devKernel, phstKernelSize[ kernelSet ] * phstKernelSize[ kernelSet ] * kernelsPerSet *
							sizeof( cufftComplex ), "creating device memory for the kernels" );
				...
				if (pGridDegrid == GRID)
				{
					...
					devGridVisibilities<<< _gridSize2D, _blockSize2D, sharedMemSize >>>
							(	/* pGrid = */ pdevGrid,
								/* pVisibility = */ &pdevVisibility[ firstVisibility ],
								/* pVisibilitiesPerBlock = */ _itemsPerBlock,
								/* pBlocksPerVisibility = */ _blocksPerItem,
								/* pGridPosition = */ &pdevGridPositions[ firstVisibility ],
								/* pKernelIndex = */ (pdevKernelIndex != NULL ? &pdevKernelIndex[ firstVisibility ] : NULL),
								/* pWeight = */ (pdevWeight != NULL ? &pdevWeight[ firstVisibility ] : NULL),
				...
				char * pCleanImageFilename, char * pResidualImageFilename )
{
	...
	while (reachedLimit == false)
	{
		...
		*phstDirtyImage = NULL;
		...
		reserveGPUMemory( (void **) &devModelImage, _hstUvPixels * _hstUvPixels * sizeof( float ), "creating memory for the model image" );
		...
		// clear the model image on the device.
		cudaMemset( devModelImage, 0, _hstUvPixels * _hstUvPixels * sizeof( float ) );
...
//moveDeviceToHost( (void *) hstModelImage, (void *) devModelImage, (long int) _hstUvPixels * (long int) _hstUvPixels * (long int) sizeof( float ), "moving model image to host" );
...
//}
		...
		cufftComplex * devDirtyImageGrid = NULL;
		reserveGPUMemory( (void **) &devDirtyImageGrid, _hstUvPixels * _hstUvPixels * sizeof( cufftComplex ),
					"reserving device memory for dirty image grid (cleaning)" );
		zeroGPUMemory( devDirtyImageGrid, _hstUvPixels * _hstUvPixels * sizeof( cufftComplex ),
...
//						cudaMemcpy( devModelVisibilities, devOriginalVisibilities, hstVisibilityBatchSize * sizeof( cufftComplex ), cudaMemcpyDeviceToDevice );
			...
			if (_hstUVMosaic == false || image == _numMosaicImages - 1)
			{
				...
				float * devDirtyImage = (float *) devDirtyImageGrid;
				...
				char * pTableData, double * phstTotalWeightPerCell )
{
	...
	if (_hstBeamPattern[0] != '\0')
		loadPrimaryBeam(	/* pBeamFilename = */ _hstBeamPattern,
	...
	if (_hstFileMosaic == true)
	{
		...
		if (_hstTelescope == ASKAP && _hstMosaicID[ pFileIndex ] != -1)
...
int main( int pArgc, char ** pArgv )
{
	...
	// set the device.
	cudaSetDevice( _hstGPU );
	
	// get some properties from the device.
	cudaDeviceProp gpuProperties;
	cudaGetDeviceProperties( &gpuProperties, _hstGPU );
...
//			strcpy( hstFilename[ i ], _hstMeasurementSetPath[ i ] );
//		else
//			strcpy( hstFilename[ i ], "\0" );
...
// cjs-mod				_hstNormalisationPattern[ index ] += pow( beamPtr[ index ], 2 );
	...
	cufftComplex * devDirtyBeamGrid = NULL;
	if (_hstMinorCycles > 0)
	{

		// create memory for the psf on the device, and clear it.
		reserveGPUMemory( (void **) &devDirtyBeamGrid, _hstUvPixels * _hstUvPixels * sizeof( cufftComplex ), "declaring device memory for psf" );
		zeroGPUMemory( (void *) devDirtyBeamGrid, _hstUvPixels * _hstUvPixels * sizeof( cufftComplex ), "zeroing the dirty beam on the device" );
	...
	float * devDirtyBeam = (float *) devDirtyBeamGrid;
	...
	cufftComplex * devDirtyImageGrid = NULL;
	reserveGPUMemory( (void **) &devDirtyImageGrid, _hstUvPixels * _hstUvPixels * sizeof( cufftComplex ), "declaring device memory for grid" );
	zeroGPUMemory( (void *) devDirtyImageGrid, _hstUvPixels * _hstUvPixels * sizeof( cufftComplex ), "zeroing the grid on the device" );
	...
	for ( int image = 0; image < _numMosaicImages; image++ )
	{
		...
		if (_hstUVMosaic == false || image == _numMosaicImages - 1)
		{
			...
			for ( int i = 0; i < stages; i++ )
			{
				...
				float * devDirtyImageDbl = (float *) devDirtyImageGrid;
			...
			// divide the dirty image by the deconvolution image.
			devDivideImages<<< _gridSize2D, _blockSize2D >>>(	/* pOne = */ (float *) devDirtyImageGrid,
										/* pTwo = */ _devDeconvolutionImage,
										/* pMask = */ NULL,
										/* pSizeOne = */ _hstUvPixels,
										/* pSizeTwo = */ _hstPsfSize );
			...
			if (image == _numMosaicImages - 1)
			{
				...
				moveDeviceToHost( (void *) hstDirtyImage, (void *) devDirtyImageGrid, _hstUvPixels * _hstUvPixels * sizeof( float ),
			...
			if (_hstFileMosaic == true)
				moveDeviceToHost( (void *) &hstDirtyImageCache[ (long int) _hstUvPixels * (long int) _hstUvPixels * (long int) image ],
							(void *) devDirtyImageGrid, _hstUvPixels * _hstUvPixels * sizeof( float ),
							"copying dirty image from device" );
